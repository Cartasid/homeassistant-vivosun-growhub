"""Tests for clearing persistent AeroLush desired shadow state."""
...
async def test_clear_aerolush_desired_publishes_null_aircd_only(
    hass: HomeAssistant, monkeypatch: MonkeyPatch
) -> None:
    ...
    coordinator.async_publish_shadow_update.assert_awaited_once_with(
        {"state": {"desired": {"aircd": None}}},
        device_id=_AC_DEV_ID,
    )


async def test_clear_aerolush_desired_rejects_non_aerolush_device(
    hass: HomeAssistant, monkeypatch: MonkeyPatch
) -> None:
    _, coordinator = await _setup(hass, monkeypatch)
...
async def test_clear_aerolush_desired_service_removed_on_unload(
    hass: HomeAssistant, monkeypatch: MonkeyPatch
) -> None:
    entry, _ = await _setup(hass, monkeypatch)
    assert hass.services.has_service(DOMAIN, SERVICE_CLEAR_AEROLUSH_DESIRED) is True

    monkeypatch.setattr(hass.config_entries, "async_unload_platforms", AsyncMock(return_value=True))
    assert await async_unload_entry(hass, entry)
    assert hass.services.has_service(DOMAIN, SERVICE_CLEAR_AEROLUSH_DESIRED) is False
